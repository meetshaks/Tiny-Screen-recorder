...
class ScreenRecorderApp:
    def __init__(self, root):
        ...
        # Force the window to appear in the taskbar despite overrideredirect.
        # overrideredirect(True) hides the taskbar button by default on Windows.
        # We use the Win32 API to set the extended window style back to include
        # WS_EX_APPWINDOW which forces a taskbar button.
        ...
        self.segments = []
        self.mic_device = None  # Will be set after device list is fetched
    ...
    def _force_taskbar_icon(self):
        """Force the window to show in the Windows taskbar even with overrideredirect(True)."""
        try:
            GWL_EXSTYLE    = -20
            ...
            # Remove ToolWindow flag (hides from taskbar), add AppWindow flag (shows in taskbar)
            style = (style & ~WS_EX_TOOLWINDOW) | WS_EX_APPWINDOW
            ctypes.windll.user32.SetWindowLongW(hwnd, GWL_EXSTYLE, style)
            # Hide and show to refresh the taskbar
    ...
    def on_splash_complete(self):
        self.root.deiconify()
        # Force taskbar icon after window is visible
        self.root.after(200, self._force_taskbar_icon)
    ...
    def setup_ui(self):
        ...
        # Row: Microphone
        self.mic_var = tk.IntVar(value=0)
        mf = create_card_row(card, "Microphone")
        self.lbl_mic_on = tk.Label(mf, text=" ON ", bg="#252636", fg=self.muted,
                                   font=("Segoe UI", 8, "bold"), cursor="hand2", padx=5)
        self.lbl_mic_on.pack(side=tk.LEFT, padx=(0, 2))
        self.lbl_mic_off = tk.Label(mf, text=" OFF ", bg=self.red, fg="white",
                                    font=("Segoe UI", 8, "bold"), cursor="hand2", padx=5)
        self.lbl_mic_off.pack(side=tk.LEFT)
        self.lbl_mic_on.bind("<Button-1>",  lambda e: self.set_mic(1))
        self.lbl_mic_off.bind("<Button-1>", lambda e: self.set_mic(0))

        tk.Frame(card, bg="#252636", height=1).pack(fill=tk.X, padx=10)

        # Row: Mic Volume
        self.mic_boost = tk.DoubleVar(value=3.0)
        vf = create_card_row(card, "Mic Volume")
        self.lbl_mic_vol = tk.Label(vf, text="3x", bg=self.card_color, fg=self.accent,
                                    font=("Segoe UI", 8, "bold"), width=3)
        self.lbl_mic_vol.pack(side=tk.RIGHT, padx=(4, 0))
        mic_slider = tk.Scale(
            vf, from_=1.0, to=8.0, resolution=0.5,
            orient=tk.HORIZONTAL, variable=self.mic_boost,
            bg=self.card_color, fg=self.muted, troughcolor="#252636",
            highlightthickness=0, bd=0, sliderlength=12, length=110,
            showvalue=False,
            command=lambda v: self.lbl_mic_vol.config(text=f"{float(v):.1g}x")
        )
        mic_slider.pack(side=tk.RIGHT)
    ...
    def set_mic(self, value):
        self.mic_var.set(value)
        if value == 1:
            # Try to find a microphone device if not already found
            if not self.mic_device:
                self.mic_device = self._get_default_mic()
            if not self.mic_device:
                messagebox.showwarning(
                    "No Microphone Found",
                    "No microphone device was detected.\n\n"
                    "Make sure your earphone/headset is plugged in\n"
                    "and Windows has microphone access enabled.\n\n"
                    "Check: Settings → Privacy → Microphone"
                )
                return
            self.lbl_mic_on.config(bg=self.green,  fg="white")
            self.lbl_mic_off.config(bg="#252636", fg=self.muted)
        else:
            self.lbl_mic_on.config(bg="#252636", fg=self.muted)
            self.lbl_mic_off.config(bg=self.red,  fg="white")

    def _get_default_mic(self):
        """Query FFmpeg DirectShow devices and return first microphone name, or None."""
        ...
        try:
            ...
            import re
            # Strategy 1: Find lines explicitly tagged as (audio) — most reliable
            # FFmpeg lines look like:  [dshow @ 0x...] "Microphone (Realtek)" (audio)
            for line in output.splitlines():
                if "(audio)" in line:
                    m = re.search(r'"([^"]+)"', line)
                    if m:
                        name = m.group(1)
                        if "virtual-audio-capturer" not in name.lower():
                            return name

            # Strategy 2: Fallback — scan audio section between headers
            in_audio_section = False
            for line in output.splitlines():
                low = line.lower()
                if "audio device" in low or "audio input" in low:
                    in_audio_section = True
                    continue
                if in_audio_section:
                    if "video device" in low or "video input" in low:
                        break
                    m = re.search(r'"([^"]+)"', line)
                    if m:
                        name = m.group(1)
    ...
    def actual_start_recording(self):
        ...
        # Microphone input (separate dshow source)
        if self.mic_var.get() == 1 and self.mic_device:
            cmd.extend([
                "-f", "dshow",
                "-probesize", "32",
                "-analyzeduration", "0",
        ...
        # Audio mixing: decide how many audio inputs we have
        has_sys = self.audio_var.get() == 1
        has_mic = self.mic_var.get() == 1 and bool(self.mic_device)
        if has_sys and has_mic:
            # Boost mic 3x to balance with system audio, then mix
            cmd.extend([
                "-filter_complex",
                f"[1:a]volume=1.0[sys];[2:a]volume={self.mic_boost.get()}[mic];[sys][mic]amix=inputs=2:duration=first:normalize=0[aout]",
                "-map", "0:v", "-map", "[aout]"
            ])
