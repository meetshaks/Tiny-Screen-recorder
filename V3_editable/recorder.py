...
class ScreenRecorderApp:
    ...
    def _setup_hotkeys(self):
        # Store the thread ID so we can send it a quit message later
    ...
    def close_app(self):
        ...
        # Signal the hotkey thread to exit its message loop
        if hasattr(self, 'hotkey_tid'):
            ctypes.windll.user32.PostThreadMessageW(self.hotkey_tid, 0x0012, 0, 0)
            
        # Unregister hotkeys for this thread (if any)
        try:
        ...
        # Hard exit to ensure all background processes and threads are killed
        os._exit(0)
